...
export function DashboardNodeLibraryView({
  props
}: DashboardNodeLibraryViewProps) {
      ...
      <div className="flex h-full flex-col justify-between rounded-xl border border-amber-300/30 bg-amber-500/10 p-4 text-sm text-amber-100">
        <div>
          ...
          <p className="mt-2 text-xs text-amber-50/80">
            Select a product in the page editor so this node can render library
            assets.
          </p>
        </div>
  ...
  return (
    <article className="flex h-full flex-col justify-between rounded-[var(--tenant-node-radius)] p-4">
      <div>
        ...
        <p className="mt-3 text-sm text-zinc-400">
          Source node that expands the product library into real page-grid
          items.
        </p>
      </div>
